...
// 	for(int i=0;i<100;i++)
	{
		...
		for(int i1=0;i1<ed.cname->size();i1++)
		{
			...
			isPhaseIHEP17=false;if(RunNo>=287582 && MP[MI].iphi>=63 && MP[MI].iphi<=66 && MP[MI].ieta>0) isPhaseIHEP17=true;
			if(isPhaseIHEP17)
			{
	...
	for(int i2=0;i2<CD.size();i2++)
	{
		...
		nmaxOK=0;
		isPhaseIHEP17=false;if(RunNo>=287582 && MP[CD[i2].mapind].iphi>=63 && MP[CD[i2].mapind].iphi<=66 && MP[CD[i2].mapind].ieta>0) isPhaseIHEP17=true;
		for(int i4=0;i4<4;i4++)
		{
			if(CD[i2].max[i4]>0.015 && !isPhaseIHEP17) nmaxOK++;
			if(CD[i2].max[i4]>0.05 && isPhaseIHEP17) nmaxOK++;
		}
		if(nmaxOK>1)
		{
			for(int i4=0;i4<4;i4++)
